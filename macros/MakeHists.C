...
Double_t Accept( TVector3 vec, TF2* beamFunc, TF2* colFunc, Double_t colDist );
TRandom3 *randoms = 0;
...
void MakeHists(){
  ...
  TFile* outFile = new TFile("outPlotsTemp0.root","RECREATE");
...
//   TH1F* polHistOffAPol = new TH1F("polhistOffApol","polhistOffApol",ebins,emin,emax);
  ...
  // Beam shape gaus function
  auto beamFunc = new TF2("beam","bigaus");
  
  Double_t xBeamRad = 0.0002;  //m
  Double_t yBeamRad = 0.00006; //m

  beamFunc->SetParameters(1,0,xBeamRad,0,yBeamRad);

  // Collimator shape "elipse step" function
  auto colFunc = new TF2("col","(x*x/([0]*[0]) + y*y/([1]*[1]) < 1) ? 1 : 0");
  
  Double_t xColRad   = 0.001; //m
  Double_t yColRad   = 0.001; //m
  Double_t colRot    = 0.0;   //deg

  colFunc->SetParameters(xColRad,yColRad);//,colRot); //expand to rotate ellipse too

  //Distance to collimator
  Double_t colDist = 2.51;
...
//     cout << "HI" << endl;
    ...
    //if(qpho>-0.000001){
    if(Accept(vec,beamFunc,colFunc,colDist)){
      Double_t e = k[2];
      Double_t weight = (*wgt);
...
Double_t Accept( TVector3 vec, TF2* beamFunc, TF2* colFunc, Double_t colDist ){ 
  ...
  beamFunc->GetRandom2(xBeam,yBeam);

  TVector3 colPlanePos = vec*colDist+TVector3(xBeam,yBeam,0);
 
  return colFunc->Eval(colPlanePos.x(),colPlanePos.y());  
