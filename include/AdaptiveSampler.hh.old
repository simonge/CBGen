...
//    where the vectors u_i in the above sum are unifomrly sampled
...
//    and the weights w_i compensates for the non-uniformity by
...
//    sum gives result_IS. If the user feeds back the integral(u_i) to
...
//    u variables ~Unif(0,1) are needed to completely specify an point in
