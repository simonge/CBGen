...
AdaptiveSampler::AdaptiveSampler(int dim, TRandom3* user_generator, int nfixed)
 : fNdim(dim),
   fNfixed(nfixed),
   fMaximum_depth(30),
   fMaximum_cells(1000000),
   fSampling_threshold(0.005),
...
double AdaptiveSampler::sample(double *u)
{
   ...
   fRandom->RndmArray(fNdim - fNfixed + 1, uu + fNfixed);
...
//       std::cout << i << " " << depth << " " << uu[i] << " " << u0[i] << " " << u1[i] << " " << u[i] << std::endl;
   }
   //std::cout << cell->subset << std::endl << std::endl;
   delete [] u0;
   delete [] u1;
   delete [] uu;
   ...
   //std::cout <<cell->opt_subset << " " << dNu << std::endl << std::endl;
                                                 ...
                                                 const double *u)
{
   ...
   //std::cout << sel << std::endl << std::endl; 
   return sel;
}
...
int AdaptiveSampler::recursively_update(std::vector<int> index)
{
          ...
          depth < fMaximum_depth)
      {
         ...
         if (verbosity > 3) {
            std::cout << "nope, efficiency is " << efficiency
                      << ", already meets the target value!"                      << std::endl;
         }
      }
      else {
         if (verbosity > 3) {
   ...
   //std::cout << cell->subset << " " << cell->opt_subset << std::endl;
   return count;
}
...
void AdaptiveSampler::Cell::optimize()
{
  ...
  //std::cout << sum_wI2s << " " << subset << " " << opt_subset << std::endl;
  //std::cout << nhit << " " << opt_subset<< std::endl;
   ...
   //std::cout << opt_subset << std::endl<< std::endl;
 
   //subset = opt_subset; //ADDED!!!
}
...
int AdaptiveSampler::Cell::deserialize(std::ifstream &ifs, double subset_multiplier)
{
   ...
   //std::cout << nhit << " " << opt_subset << " "  << subset_multiplier << " " << count << std::endl;
   return count;
}
